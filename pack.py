...
with open(headerPart, 'wb') as header:
	if headerScriptLabel:
		...
		header.write('# Device : \n'.encode())
		header.write('# Build PATH : /home/workspace\n'.encode())
		# CurrentTime = '# Build TIME : ' + headerScriptLabel.format(time=today) + '\n'
		# header.write(CurrentTime.encode())
		# TimeStamp = '# Build TIME STAMP : ' + headerScriptLabel.format(timestamp=int(today.timestamp())) + '\n'
		# header.write(TimeStamp.encode())
		header.write('\n'.encode())
...
#	header.write('# Partitions'.encode())
	for sectionName in parts:
		...
		if (type == 'partitionImage'):
			...
			if sparse:
				if (name == 'cache') or (name == 'userdata'):
					print ('[i]      Restory cache or userdata sparse ...')
					chunks = utils.userdata_restory(name, projectFolder, tmpDir)
			...
			for index, inputChunk in enumerate(chunks):
				...
				if lzo:
					...
					if  ('cache' in inputChunk) or ('userdata' in inputChunk):
